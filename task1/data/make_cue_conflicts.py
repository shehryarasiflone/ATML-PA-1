...
def transfer_style(vgg_net, content_img, style_img, steps=45, content_weight=1.0, style_weight=8e4):
    with torch.no_grad():
...
def generate_cue_conflicts():
    ...
    target_per_dir = 21  # 5 pairs * 2 directions * 21 = 210 conflicts (>= 200 target)
    metadata = []
    rejection_counts = {"valid": 0, "structural_collapse": 0, "identity_failure": 0, "low_contrast": 0}
    to_tensor = T.ToTensor()

    print("Generating cue-conflict images via VGG style transfer...")
    ...
    for c1_name, c2_name in class_pairs:
        ...
        for content_cls, c_name, style_cls, s_name in directions:
            ...
            while accepted_count < target_per_dir and i < len(c_imgs):
                ...
                stylized_t = transfer_style(vgg_net, c_t, s_t, steps=45, content_weight=1.0, style_weight=8e4)
    ...
    print(f"Accepted Valid Conflicts : {len(metadata)}")
